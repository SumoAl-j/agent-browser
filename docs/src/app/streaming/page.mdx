...
Input handling is independent of frame delivery. Each connection reads input on its own task, so clicks and keystrokes dispatch to the browser immediately even while a large frame is mid-write to a slow client.
...
Under ack pacing the frame rate is bounded by one frame per acknowledgement round trip, so a client that queues heavy input on the same socket delays its own acks and throttles its own stream. Measured: input above roughly 120 messages per second on the same connection is enough to do it. Send acks ahead of buffered input, or use push pacing.

...
Under ack pacing the frame rate is bounded by one frame per acknowledgement round trip, so it is limited by the link's latency rather than its bandwidth.

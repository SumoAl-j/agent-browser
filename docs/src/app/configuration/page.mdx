...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>useSystemCa</code></td><td><code>--use-system-ca</code></td><td>boolean</td></tr>
...
`caCert` adds the supplied CA certificate or PEM bundle to the CLI's own TLS trust. For locally launched Chromium on Linux, it also imports each certificate into an isolated NSS database used only by that browser process. Browser trust remains active when later commands omit it, and different certificate content or `clearCaCert: true` relaunches Chromium without restarting the daemon. `agent-browser install --with-deps` installs the required `certutil`; otherwise install `libnss3-tools` on Debian/Ubuntu or `nss-tools` on RPM Linux. Browser-side trust does not support Chrome profiles, Lightpanda, macOS, or Windows. With CDP attach, auto-connect, or providers, `caCert` affects only CLI TLS and cannot change the remote browser's trust store. Set `useSystemCa` when the CA already lives in the operating system trust store.

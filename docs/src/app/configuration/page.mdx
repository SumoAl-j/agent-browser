...
`caCert` adds the supplied CA certificate or PEM bundle to the CLI's own TLS trust. For locally launched Chromium on Linux, it also imports each certificate into an isolated NSS database used only by that browser process; install `certutil` from `libnss3-tools` first. Browser-side trust does not support Chrome profiles, Lightpanda, macOS, or Windows. With CDP attach, auto-connect, or providers, `caCert` affects only CLI TLS and cannot change the remote browser's trust store. Set `useSystemCa` when the CA already lives in the operating system trust store.

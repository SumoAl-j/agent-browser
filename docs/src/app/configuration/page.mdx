...
`caCert` imports the supplied CA certificate or PEM bundle into an isolated NSS database for locally launched Chromium on Linux. Hostname, validity period, and unrelated CA verification remain enabled. Install `certutil` from `libnss3-tools` first. This initial implementation does not support Chrome profiles, CDP attach, providers, Lightpanda, macOS, or Windows.
